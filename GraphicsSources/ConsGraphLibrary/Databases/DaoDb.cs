...
namespace BaseLibrary
{
    ...
    public class DaoDb : IDisposable
    {
            ...
            IndexModes indexMode = IndexModes.WithoutChange, string defaultValue = null, bool required = false, bool emptyStrings = true)
        {
            ...
            ExecuteAdo("ALTER TABLE " + tableName + " ALTER COLUMN [" + columnName + "] TEXT(" + length +
                       ") WITH COMPRESSION");
            Dispose();
        ...
        public void AddForeignLink(string tableName, string columnName, string linkedTable, string linkedColumn, bool cascade = true)
        {
            ...
            ExecuteAdo("ALTER TABLE " + tableName + " ADD CONSTRAINT " + linkedTable + tableName
                       + " FOREIGN KEY ([" + columnName + "]) REFERENCES " + linkedTable + "(" + linkedColumn +
                       ")" + cascadeS);
        }
        ...
        {
            int paramId;
            using (var sysTablRS = new RecDao(templatePath,
                                           "SELECT ParamId,ParamName FROM SysTabl WHERE ParamName='" + paramName + "'"))
            {
            ...
            Execute("INSERT INTO SysSubTabl " +
                    ...
                    "FROM [" + templatePath + "].SysSubTabl t1 WHERE t1.ParamId=" + paramId + " AND t1.SubParamName='" +
                    subParamName +
