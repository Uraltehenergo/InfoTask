...
namespace CommonTypes
{
    public partial class ProviderSetupForm : Form
    {
                    ...
                    while (rec.Read())
                    {
                        ...
                        if (b)
                        {
                            ...
                            if (!rowSource.IsEmpty() || tag.ContainsKey("SpecialComboBox"))
                            {
                                ...
                                else list = rowSource.Split(';').ToList();
                                if (list.Count > 0)
                                {
                                    cell.Value = list[0];
        ...
        private void ButOK_Click(object sender, EventArgs e)
        {
            ...
            string err;
            try { err = Provider.CheckSettings(dic, _namesDic); }
            catch (Exception ex) { err = "Ошибка соединения с провайдером" + Environment.NewLine + ex.Message + Environment.NewLine + ex.StackTrace; }
            if (err.IsEmpty() || Different.MessageQuestion(err + (err.Last() =='\n' ? "" : "\n") + "Закрыть форму настроек?", "Недопустимые настройки"))
            {
