    ...
    public abstract class OpcServer : ProviderBase, IReceiver
    {
        ...
        public bool Connect()
        {
            ...
            try
            {
                ...
                {
                    Logger.AddError("Ошибка соединения с OPC-сервером", null , "Состояние = " + State, Context);
                    return IsConnected = false;
                }
            }
            catch (Exception ex)
            {
                Logger.AddError("Ошибка соединения с OPC-сервером", ex, "", Context);
                return IsConnected = false;
            }
        ...
        public bool Check()
        {
            return Logger.Danger(Connect, 2, 500, "Ошибка соединения с OPC-сервером");
        }
        ...
        public bool CheckConnection()
        {
            ...
            Logger.AddError(CheckConnectionMessage = "Ошибка соединения с OPC-сервером");
            return false;
        }
        ...
        public ProviderSignal AddSignalByTag(string tag, DataType dataType, string v)
        {
            ...
            item.Tag = tag;
            item.ClientHandler = _signals.Count;
            item.Value = new SingleValue(new Moment(dataType, v));
            if (!_items.ContainsKey(item.Tag))
                _items.Add(item.Tag, item);
            return item;
        }
        ...
        private bool TryPrepare()
        {
            ...
            {
                Logger.AddError("Ошибка подготовки серверной группы", ex);
                return IsConnected = false;
            }
        }
        public void Prepare()
        {
            ...
            else Logger.Danger(TryPrepare, 2, 1000, "Ошибка подготовки OPC-сервера");
        }
        ...
        {
            if (!Logger.IsRepeat) Logger.Danger(TryWriteValues, 2, 500, "Ошибка записи значений в OPC-сервер");
            else Logger.Danger(() => Logger.Danger(TryWriteValues, 2, 500, "Ошибка записи значений в OPC-сервер"), 3, 10000, "Ошибка подготовки OPC-сервера", TryPrepare);
        }
    }
